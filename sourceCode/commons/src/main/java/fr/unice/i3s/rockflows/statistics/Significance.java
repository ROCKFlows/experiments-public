package fr.unice.i3s.rockflows.statistics;
...
public class Significance {
    ...
    public static boolean isSignificantDifferent(double[] array1, double[] array2, double alpha, boolean exactPValue)
            throws Exception {
        ...
        if (normal1 && normal2) {
            ...
            return wilcoxonSignedTest(array1, array2, alpha, exactPValue);
        }
    }
    ...
    public static boolean kolmogorovSmirnovNormalityTest(double[] array, double alpha) {

        double mean = StatUtils.mean(array);
        //compute standard deviation of data
        double variance = StatUtils.variance(array, mean);
        double sd = Math.sqrt(variance);
        KolmogorovSmirnovTest kst = new KolmogorovSmirnovTest();
        boolean test = false;
        //if sd = 0 -> notStrictlyPositiveException
        if (sd <= 0 || mean <= 0) {
            return false;
        } else {
            //true iff c with confidence 1 - alpha
            test = kst.kolmogorovSmirnovTest(new NormalDistribution(mean, sd), array, alpha);
        }
        if (test) {
            //the null hypothesis that data is a normal distribution has been rejected
            //so this data is not normal
            return false;
        }
        return true;
    }
     ...
     * @param exactPValue whether computed pValue should be exact or not
     * @return {@code true} if the array are significant different (null hypothesis rejected) {@code false} otherwise
     */
    private static boolean wilcoxonSignedTest(double[] acc1, double[] acc2, double alpha, boolean exactPValue) {
        ...
        Arrays.sort(copy1);
        Arrays.sort(copy2);
        double pvalue = wsrt.wilcoxonSignedRankTest(copy1, copy2, exactPValue);
        /*
     ...
     * @param alpha the level of confidence of the test. Between 0 and 0.5
     * @param exactPValue whether computed pValue should be exact or not for wilcoxonSignedTest
     * @return {@code true} if the array are significant different (null hypothesis rejected) {@code false} otherwise
     */
    public static boolean friedmanTest(List<double[]> accs, double alpha, boolean exactPValue) {
        int count = accs.size();
        for (int iii = 0; iii < count; iii++) {
            for (int jjj = iii + 1; jjj < count; jjj++) {
                //compare accuracies i and j with Wilcoxon
                boolean wt = wilcoxonSignedTest(accs.get(iii), accs.get(jjj), alpha, exactPValue);
                if (wt) {
